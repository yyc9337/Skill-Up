(function ($) {
    ...
    function bindData(template, data, settings) {
        ...
        processElements("data-content2", template, data, settings, function ($elem, value) {
            $elem.html(applyFormatters2($elem, data, "content", settings));
        });
        ...
        processElements("data-ondblclick", template, data, settings, function ($elem, value) {
            $elem.attr("ondblclick", $elem.attr("ondblclick")+"('"+value+"');")
        });
        
        processElements("data-onmouseover", template, data, settings, function ($elem, value) {
            $elem.attr("onmouseover", $elem.attr("onmouseover")+"('"+value.eventNo+"|"+value.eventType+"');") //하드코딩 수정 필요...
        });
    ...
    function applyFormatters2($elem, data, attr, settings) {
        var formatterTarget = $elem.attr("data-format-target"),
            formatter;
        ...
        if (formatterTarget === attr || (!formatterTarget && attr === "content")) {
            ...
            if (formatter && typeof formatters[formatter] === "function") {
                var formatOptions = $elem.attr("data-format-options");
                return (function (formatterSettings) {
                    return formatters[formatter].call($elem[0], data, formatOptions, $.extend({}, formatterSettings));
                })(settings);
            }
        }
